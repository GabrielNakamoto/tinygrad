...
def cmp(x:UOp) -> UOp:
  ...
  return x.ins(X86Ops.CMP, dtype=dtypes.void) if (i:=to_imm(x.src[1])) is None else x.ins(X86Ops.CMPi, x.src[0], i, dtype=dtypes.void)
...
def flag_rematerialize(ctx:X86LinearContext, x:UOp):
  if x.op in (Ops.RANGE, Ops.END) or x.arg.opcode in X86GroupOp.WriteFlags: ctx.lock = x
  elif x.arg.opcode in X86GroupOp.ReadFlags and ctx.lock is not (flag_def:=x.src[-1]):
    ctx.lock = flag_def
    return (x, [flag_def, x])
  return None
...
class X86Renderer(ISARenderer):
  ...
  def asm_str(self, uops:list[UOp], function_name:str) -> str:
    ...
    def _format_operands(x:UOp) -> str:
      ...
      if len(x.src) > 4 and x.arg.opcode in X86GroupOp.WriteMem: ret = _mem_adress(*x.src[:4]) + _format(x.src[4:])
      elif len(x.src) > 3 and x.arg.opcode in X86GroupOp.Rm1st: ret = _format((x,)) + _mem_adress(*x.src[:4]) + _format(x.src[4:])
      elif len(x.src) > 4 and x.arg.opcode in X86GroupOp.Rm2nd: ret = _format((x, x.src[0])) + _mem_adress(*x.src[1:5]) + _format(x.src[5:])
  ...
  def render(self, uops:list[UOp]) -> str:
    ...
    for u in uops:
      ...
      binary.extend(l)
      if u.arg.opcode in (X86Ops.JL, X86Ops.JB, X86Ops.JE, X86Ops.JNE, X86Ops.JGE, X86Ops.JMP): jumps[u] = len(binary)
