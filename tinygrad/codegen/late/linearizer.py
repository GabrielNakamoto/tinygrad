...
def linearize(sink:UOp, tuple_order:bool=True) -> list[UOp]:
  ...
  nkey = {u:i for i,u in enumerate(sorted(lst, key=lambda x: priorities[x]+(x.tuplize if tuple_order and TUPLE_ORDER else ())))}
