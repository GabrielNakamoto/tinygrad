...
spec_shared = PatternMatcher([
  ...
  (UPat(Ops.GROUP, dtypes.void, src=UPat((Ops.GROUP, Ops.STORE, Ops.NOOP, Ops.END, Ops.CALL))), lambda: True),
  ...
  # CALL: the body is always an opaque body, the arg is a CallInfo/InstInfo stating the (possibly void) dtype
  (UPat(Ops.CALL, src=(UPat(tuple(OPAQUE_CALL_BODIES)),), allow_any_len=True, name="x"),
   lambda x: isinstance(x.arg, (CallInfo, InstInfo)) and x.dtype is x.arg.dtype),
