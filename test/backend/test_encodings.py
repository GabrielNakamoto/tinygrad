...
def ptr(r:Register, dt:DType) -> UOp: return UOp(Ops.NOOP).bitcast(dt).ins(X86Ops.DEFINE, tag=(r,))
...
class TestEncodingsX86(unittest.TestCase):
  ...
  def test_cmove_ignore_cmp(self):
    cmove = ins(X86Ops.CMOVE, dtypes.int32, (def_reg(reg(RAX, 4)), ins(X86Ops.CMP, dtypes.void, ())), reg(RDX, 4))
    # cmove edx, eax
